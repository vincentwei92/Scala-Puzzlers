...
object Sorting {
  ...
  def stableSort[K](a: Array[K], f: (K, K) => Boolean): Unit = println(f)
  
  def ltWithNaNGtAll(x: Double, y: Double) =
      (x compare y) < 0

  stableSort(Array(1.0, 2.0), ltWithNaNGtAll)

  def testSort[T](a: Array[T], f: (T, T) => Boolean): Unit = println(f)
  def testSort[T](a: Seq[T], f: T): Unit = println(f)

  testSort(Array(1.0,2.0,3.0), ltWithNaNGtAll)
